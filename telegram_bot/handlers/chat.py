...
async def confirm_completion_customer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Завершить (задание и произвести оплату
    if chat.performer_confirmed_completion and chat.customer_confirmed_completion:
        ...
        if listing.payment_method.value == 'prepayment':
            # Звёзды уже списаны при создании Задания
            # Перевести их исполнителю
            performer = service.get_user(chat.performer_id)
            performer.stars_balance += int(listing.price)
            db.commit()
        elif listing.payment_method.value == 'after_completion':
            # Списать звёзды у заказчика и перевести исполнителю
            customer = service.get_user(chat.customer_id)
            performer = service.get_user(chat.performer_id)
            
            if customer.stars_balance >= listing.price:
                customer.stars_balance -= int(listing.price)
                performer.stars_balance += int(listing.price)
                db.commit()
