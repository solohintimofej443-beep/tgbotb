...
from config import BOT_TOKEN, PROXY_URL, PROXY_LIST, PROXY_JSON_URL
...
def main():
    ...
    # Создать приложение (с прокси, если задан)
    builder = Application.builder().token(BOT_TOKEN)

    def load_proxies_from_url(url: str) -> list:
        if not url:
            return []
        try:
            ...
            logger.warning(f"Failed to load proxy list from URL: {e}")
            return []
        proxies = []
    ...
    def test_proxy(candidate: str) -> bool:
        try:
            with httpx.Client(proxy=candidate, timeout=6.0) as client:
                client.get("https://api.telegram.org", follow_redirects=True)
            return True
        except Exception:
            return False
    ...
    candidates.extend(load_proxies_from_url(PROXY_JSON_URL))
    ...
    if proxy_url:
        ...
        logger.warning("No working proxy found. Starting without proxy.")
